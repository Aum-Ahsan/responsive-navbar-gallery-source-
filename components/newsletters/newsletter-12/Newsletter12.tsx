...
export default function Newsletter12() {
  ...
  return (
    ...
    <div className="w-full p-4 sm:p-8 my-10 font-sans">
      <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden relative min-h-[240px] sm:h-[200px] sm:h-[260px] lg:h-[300px] lg:h-[350px]">
        {/* Progress Bar */}
        <div className="absolute top-0 left-0 h-2 bg-blue-600 transition-all duration-500" style={{ width: `${step * 33.33}%` }}></div>
        
        <div className="p-8 sm:p-12 relative h-full flex flex-col">
          {step === 1 && (
            <div className="animate-in slide-in-from-right duration-500 flex-1">
              <div className="w-12 h-12 bg-slate-50 text-blue-600 rounded-xl flex items-center justify-center mb-6"><Mail size={24} /></div>
              ...
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@company.com" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 text-lg mb-6" />
              <button onClick={() => email && setStep(2)} className="w-full py-4 bg-slate-900 hover:bg-black text-white font-bold rounded-xl flex justify-center items-center gap-2">Continue <ArrowRight size={20} /></button>
            </div>
          )}
          {step === 2 && (
            <div className="animate-in slide-in-from-right duration-500 flex-1">
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                {['Design', 'Engineering', 'Product', 'News'].map(topic => (
                  <label key={topic} className="flex items-center gap-3 p-4 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50">
                    <input type="checkbox" className="w-5 h-5 accent-blue-600" defaultChecked={topic==='Design'} />
                    <span className="font-semibold text-slate-700">{topic}</span>
                  </label>
                ))}
              </div>
              <button onClick={() => { setStatus('loading'); setStep(3); setTimeout(() => setStatus('success'), 1500) }} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl flex justify-center items-center gap-2">Complete Signup</button>
            </div>
          )}
          {step === 3 && (
