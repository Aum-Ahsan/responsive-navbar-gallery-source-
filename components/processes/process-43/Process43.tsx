...
export default function Process43() {
  const funnel = [
    ...
    { label: 'Qualified Prospects', val: '2,500', icon: Filter, color: 'bg-indigo-600', width: 'w-4/5' },
    { label: 'Sales Meetings', val: '500', icon: Target, color: 'bg-indigo-700', width: 'w-3/5' },
    { label: 'Closed Deals', val: '120', icon: Handshake, color: 'bg-indigo-800', width: 'w-2/5' }
  ];

  return (
    <div className="w-full max-w-4xl mx-auto p-12 font-sans bg-slate-900 text-white rounded-3xl my-10">
      ...
      <div className="flex flex-col items-center space-y-2">
        {funnel.map((stage, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className={`${stage.width} ${stage.color} h-24 rounded-lg flex items-center justify-between px-6 md:px-12 transition-all duration-500 hover:scale-105 cursor-pointer shadow-lg`}
