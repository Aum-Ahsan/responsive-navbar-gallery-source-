...
export default function Carousel28() {
  ...
  return (
    <CarouselShell name="Split Transition" index={28}>
      <div className="relative aspect-[21/9] w-full rounded-2xl overflow-hidden bg-black group" style={{ perspective: '1200px' }}>
        {images.map((img, i) => {
          ...
          if ('split' === 'flip') style = { transform: `rotateY(${i === curr ? 0 : 180}deg)`, ...style };
          if ('split' === 'zoom') style = { transform: `scale(${i === curr ? 1 : 1.5})`, ...style };
          if ('split' === 'mask') style = { clipPath: i === curr ? 'circle(150% at 50% 50%)' : 'circle(0% at 50% 50%)', ...style, opacity: 1, zIndex: i === curr ? 10 : 5 };
          if ('split' === 'clip') style = { clipPath: i === curr ? 'polygon(0 0, 100% 0, 100% 100%, 0 100%)' : 'polygon(50% 50%, 50% 50%, 50% 50%, 50% 50%)', ...style, opacity: 1, zIndex: i === curr ? 10 : 5 };
          if ('split' === 'split') style = { clipPath: i === curr ? 'inset(0 0 0 0)' : 'inset(50% 0 50% 0)', ...style };
