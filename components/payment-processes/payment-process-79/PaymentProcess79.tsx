      ...
      return (
        <div className="w-full min-h-[600px] bg-gray-50 flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans">
          <div className="bg-white rounded-3xl p-8 sm:p-10 text-center max-w-md w-full shadow-xl border border-gray-100">
            ...
            <button onClick={() => setIsSuccess(false)} className="text-violet-600 font-semibold hover:opacity-80 transition-opacity text-sm sm:text-base">
              Return to Dashboard
            </button>
          </div>
        </div>
      ...
      <div className="w-full min-h-[700px] bg-gray-50 flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans">
        <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-start">
          <div className="space-y-6 md:space-y-8">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-50 text-violet-600 text-[10px] sm:text-xs font-bold tracking-widest uppercase mb-3 sm:mb-4">
                <Zap className="w-3 h-3" /> Electronics Affirm-style Checkout
              </div>
            ...
            <div className="bg-white rounded-3xl p-1.5 shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-1.5 sm:gap-0">
              <button onClick={() => setPaymentMode("upfront")} className={`flex-1 py-3 px-6 rounded-2xl text-xs sm:text-sm font-semibold transition-all ${paymentMode === "upfront" ? "bg-violet-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}>Pay in full</button>
              <button onClick={() => setPaymentMode("split")} className={`flex-1 py-3 px-6 rounded-2xl text-xs sm:text-sm font-semibold transition-all ${paymentMode === "split" ? "bg-violet-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}>Split payment</button>
            </div>
          ...
          <div className="bg-white rounded-[2rem] p-6 sm:p-8 md:p-10 shadow-xl border border-gray-100 relative overflow-hidden">
            <form onSubmit={handlePay} className="space-y-4 sm:space-y-5 relative z-10">
              <div>
                ...
                <input required type="email" placeholder="you@example.com" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 sm:py-3.5 text-sm sm:text-base text-gray-900 placeholder-gray-400 focus:outline-none transition-all focus:ring-violet-600/20 focus:border-violet-600" />
              </div>
              <div>
              ...
              <div className="pt-2 sm:pt-4">
                <button type="submit" disabled={isProcessing} className="w-full bg-violet-600 text-white font-bold py-3.5 sm:py-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed text-sm sm:text-base">
