...
def add_hyperlink(paragraph, url, text):
    ...
    c.set(docx.oxml.shared.qn('w:val'), "0563C1") # Blue color
    rPr.append(c)
...
doc.add_paragraph('This document details the complete inventory of UI components, providing specific type names, usage contexts, and clickable real-world reference links.')
...
def add_component_section(doc, category_id, section_title, prefix, count, types_list, links_list):
    ...
    for i in range(1, count + 1):
        ...
        raw_link_info = links_list[i % len(links_list)]
        if " (" in raw_link_info:
            url, desc = raw_link_info.split(" (", 1)
            desc = desc.replace(")", "")
        else:
            url = raw_link_info
            desc = "Website Link"
