      ...
      <div className="w-full min-h-[700px] bg-white flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans">
        <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 rounded-[2rem] md:rounded-3xl overflow-hidden border border-gray-200 shadow-2xl">
          <div className="bg-gray-50 p-6 sm:p-8 md:p-12 border-b md:border-b-0 md:border-r border-gray-200 flex flex-col justify-between">
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">VPN Service Checkout</h2>
              ...
              <div className="space-y-3 md:space-y-4">
                {tiers.map((tier, i) => (
                  <div key={i} onClick={() => setSelectedTier(i)} className={`cursor-pointer p-4 md:p-5 rounded-2xl border-2 transition-all ${selectedTier === i ? 'border-red-200 bg-red-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
            ...
            <form onSubmit={handlePay} className="space-y-4 sm:space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="text" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 focus:outline-none text-sm sm:text-base focus:ring-red-600/20 focus:border-red-600" />
                </div>
                <div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input required type="text" placeholder="0000 0000 0000 0000" className="w-full pl-10 sm:pl-12 bg-white border border-gray-300 rounded-xl px-4 py-3 focus:outline-none font-mono text-sm sm:text-base focus:ring-red-600/20 focus:border-red-600" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-1.5">Expiry</label>
                  <input required type="text" placeholder="MM/YY" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 focus:outline-none font-mono text-sm sm:text-base focus:ring-red-600/20 focus:border-red-600" />
                </div>
                <div>
                  <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-1.5">CVC</label>
                  <input required type="text" placeholder="123" className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 focus:outline-none font-mono text-sm sm:text-base focus:ring-red-600/20 focus:border-red-600" />
                </div>
              </div>
              <button type="submit" disabled={isProcessing} className="w-full mt-4 sm:mt-6 bg-red-600 text-white font-bold py-3.5 sm:py-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-70 text-sm sm:text-base hover:shadow-lg hover:-translate-y-0.5">
