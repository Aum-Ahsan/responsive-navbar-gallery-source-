  ...
  return (
    <CarouselShell name="Orbit" index={22}>
      <div className="flex flex-col items-center">
        <div className="relative h-[600px] w-full flex items-center justify-center overflow-hidden cursor-pointer" style={{perspective: '1000px'}} onClick={()=>setRot(r=>r+45)}>
          <div className="relative w-56 h-56 md:w-64 md:h-64 transition-transform duration-1000" style={{transformStyle: 'preserve-3d', transform: `rotateY(${rot}deg) `}}>
            {images.map((img, i) => {
              const angle = (i / images.length) * 360;
              let transform = `rotateZ(${angle}deg) translateX(200px) rotateZ(-${angle}deg)`;
              return <img key={i} src={img} className="absolute inset-0 w-full h-full object-cover rounded-2xl shadow-xl" style={{transform}} />;
            })}
          </div>
        </div>
        <p className="mt-4 pb-4 text-sm font-bold text-gray-500">Click anywhere to spin the orbit</p>
      </div>
    </CarouselShell>
  );
