...
export default function Carousel06() {
  ...
  useEffect(() => { let id: any; const play = () => { if (api) api.scrollPrev(); id = setTimeout(play, 2000); }; play(); return ()=>clearTimeout(id); }, [api]);
  
  return (
    <CarouselShell name="Auto-Reverse" index={6} headerControls={<div className="flex gap-2"><button onClick={()=>api?.scrollPrev()} className="p-3 border rounded-full"><ChevronLeft/></button><button onClick={()=>api?.scrollNext()} className="p-3 border rounded-full"><ChevronRight/></button></div>}>
