...
export default function Carousel24() {
  ...
  return (
    <CarouselShell name="Fan / Deck" index={24}>
      <div className="relative h-[600px] w-full flex items-center justify-center overflow-hidden" style={{perspective: '1000px'}} onClick={()=>setRot(r=>r+45)}>
        <div className="relative w-64 h-64 transition-transform duration-1000" style={{transformStyle: 'preserve-3d', transform: `rotateY(${rot}deg) `}}>
          {images.map((img, i) => {
            ...
            let transform = '';
            if ('fan' === 'circle' || 'fan' === 'cylinder') transform = `rotateY(${angle}deg) translateZ(300px)`;
            if ('fan' === 'orbit') transform = `rotateZ(${angle}deg) translateX(200px) rotateZ(-${angle}deg)`;
            if ('fan' === 'fan') transform = `rotateZ(${(i - images.length/2) * 15}deg) translateY(-${Math.abs(i - images.length/2)*10}px)`;
            return <img key={i} src={img} className="absolute inset-0 w-full h-full object-cover rounded-2xl shadow-xl" style={{transform}} />;
          })}
        </div>
        <p className="absolute bottom-8 text-sm font-bold text-gray-500">Click anywhere to spin the fan</p>
      </div>
    </CarouselShell>
  );
