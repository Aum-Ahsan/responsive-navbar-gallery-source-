...
function VisualBlock({ kind = "dashboard" }: { kind?: Visual }) {
  ...
  if (kind === "phone") return <div className="hero-phone"><div className="mb-3 mx-auto h-1.5 w-16 rounded-full bg-current opacity-40" /><div className="hero-phone-screen" /><div className="mt-3 grid grid-cols-3 gap-2">{[1, 2, 3].map(i => <span key={i} className="h-8 rounded-lg bg-current opacity-15" />)}</div></div>;
    ...
    <section className={`hero-surface relative w-full overflow-hidden ${themes[theme]}`}>
      ...
      `}>
        ...
        <div className={`hero-copy ${layout === "fullscreen" || layout === "fullscreen-left" ? "py-24" : ""}`}>
          ...
          {/* <h3 className={`hero-title font-semibold ${layout === "fullscreen-left" ? "text-5xl md:text-6xl" : ""}`}>{title}</h3> */}
          <h3
  className="
    hero-title
    font-semibold
