          ...
          <div className="lg:col-span-5 bg-zinc-900 rounded-[2rem] p-8 shadow-2xl border border-zinc-800 flex flex-col relative z-0">
            ...
            <form onSubmit={handlePay} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9\s]/g, "").substring(0, 19); }} pattern="[\\d\\s]{16,19}" maxLength={19} title="16 digit card number" required disabled={selectedSeats.length === 0} type="text" placeholder="Card Number" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl pl-12 pr-4 py-4 text-white focus:outline-none focus:border-rose-500 transition-colors font-mono tracking-widest text-sm disabled:opacity-50" />
              </div>
              
              <div className="flex gap-4">
                <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9\/]/g, "").substring(0, 5); }} pattern="(0[1-9]|1[0-2])\\/?([0-9]{2})" maxLength={5} title="Format: MM/YY" required disabled={selectedSeats.length === 0} type="text" placeholder="MM/YY" className="w-1/2 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:outline-none focus:border-rose-500 transition-colors font-mono tracking-widest text-center text-sm disabled:opacity-50" />
                <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9]/g, "").substring(0, 4); }} pattern="\\d{3,4}" maxLength={4} title="3 or 4 digit CVV/CVC" required disabled={selectedSeats.length === 0} type="text" placeholder="CVV" className="w-1/2 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:outline-none focus:border-rose-500 transition-colors font-mono tracking-widest text-center text-sm disabled:opacity-50" />
              </div>
