...
export default function Carousel17() {
  ...
  return (
    <CarouselShell name="Scale Focus" index={17} headerControls={<div className="flex gap-2"><button onClick={()=>api?.scrollPrev()} className="p-3 border rounded-full"><ChevronLeft/></button><button onClick={()=>api?.scrollNext()} className="p-3 border rounded-full"><ChevronRight/></button></div>}>
      <div className="overflow-hidden" ref={ref}>
        <div className="flex gap-4" style={{ perspective: '1200px' }}>
          {["https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=800&q=80","https://images.unsplash.com/photo-1499856871958-5b9627545d1a?w=800&q=80","https://images.unsplash.com/photo-1506929562872-bb421503ef21?w=800&q=80","https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800&q=80","https://images.unsplash.com/photo-1473625247510-8ceb1760943f?w=800&q=80"].map((img, i) => {
            ...
            let opacity = 1;
            let zIndex = 10;
            if ('scale' === 'scale') { transform = `scale(${1 - Math.abs(diff) * 0.5})`; opacity = 1 - Math.abs(diff); }
            if ('scale' === 'coverflow') { transform = `rotateY(${diff * -60}deg) translateZ(${Math.abs(diff) * -300}px)`; zIndex = 100 - Math.abs(Math.round(diff*100)); }
            if ('scale' === 'parallax') { transform = `translateX(${diff * 200}%)`; }
            if ('scale' === 'depth') { transform = `translateZ(${Math.abs(diff) * -500}px) scale(${1 - Math.abs(diff)*0.2})`; opacity = 1 - Math.abs(diff); }
            if ('scale' === 'tunnel') { transform = `translateZ(${diff * -1000}px) rotate(${diff * 45}deg)`; opacity = 1 - Math.abs(diff); }
