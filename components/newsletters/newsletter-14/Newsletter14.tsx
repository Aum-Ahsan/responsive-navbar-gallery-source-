    ...
    <div className="w-full p-4 sm:p-8 my-10 font-sans perspective-1000">
      <div className={`relative w-full min-h-[220px] sm:h-[180px] sm:h-[240px] lg:h-[280px] lg:h-[320px] sm:h-[260px] sm:h-[340px] lg:h-[400px] transition-transform duration-700 transform-style-3d ${status === 'success' ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute inset-0 backface-hidden bg-white border border-slate-200 shadow-2xl rounded-3xl p-6 sm:p-10 flex flex-col items-center text-center">
          <div className="w-16 h-16 bg-purple-50 text-fuchsia-600 rounded-full flex items-center justify-center mb-6"><Sparkles size={32}/></div>
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="Email address" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-fuchsia-600 text-center" />
            <button type="submit" disabled={status==='loading'} className="w-full py-4 bg-fuchsia-600 text-white font-bold rounded-xl shadow-md hover:-translate-y-1 transition-transform flex justify-center">
        ...
        {/* Back */}
        <div className="absolute inset-0 backface-hidden rotate-y-180 bg-gradient-to-br from-fuchsia-600 to-purple-900 shadow-2xl rounded-3xl p-6 sm:p-10 flex flex-col items-center justify-center text-center text-white">
