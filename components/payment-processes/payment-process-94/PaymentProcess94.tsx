        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-xl border border-amber-200">
          ...
          <form onSubmit={handlePay} className="space-y-4 mb-8">
            <div className="relative">
              ...
              <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9\s]/g, "").substring(0, 19); }} pattern="[\\d\\s]{16,19}" maxLength={19} title="16 digit card number" required type="text" placeholder="Card Number" className="w-full bg-amber-50/50 border border-amber-200 rounded-xl pl-12 pr-4 py-4 focus:outline-none focus:border-amber-500 transition-colors font-mono tracking-widest text-sm"  minLength={16} />
            </div>
            
            <div className="flex gap-4">
              <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9\/]/g, "").substring(0, 5); }} pattern="(0[1-9]|1[0-2])\\/?([0-9]{2})" maxLength={5} title="Format: MM/YY" required type="text" placeholder="MM/YY" className="w-1/2 bg-amber-50/50 border border-amber-200 rounded-xl px-4 py-4 focus:outline-none focus:border-amber-500 transition-colors font-mono tracking-widest text-center text-sm"  minLength={5} />
              <input onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9]/g, "").substring(0, 4); }} pattern="\\d{3,4}" maxLength={4} title="3 or 4 digit CVV/CVC" required type="text" placeholder="CVV" className="w-1/2 bg-amber-50/50 border border-amber-200 rounded-xl px-4 py-4 focus:outline-none focus:border-amber-500 transition-colors font-mono tracking-widest text-center text-sm"  minLength={3} />
            </div>
